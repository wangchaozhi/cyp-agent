...
| 持久化 | 运行状态支持内存/原子 JSON/PostgreSQL；OHLCV 可独立归档到 TimescaleDB |
...
要求：Go 1.25+。开发仪表盘还需要 Node.js 20.19+（或 22.12+）；启用 PostgreSQL 状态持久化或默认 OHLCV 归档时需要可访问的 PostgreSQL/TimescaleDB。
