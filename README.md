...
| [docs/quant/README.md](docs/quant/README.md) | 数学模型规格分册：公式、阈值、数据要求、验收用例 |
...
#   页面：事件流 / 待审批（点按钮人在环审批）/ 持仓 / 风控看板 / Kill Switch

# 5. 回测 & 策略扫参择优（合成历史，与实盘同一套管线，零密钥）
python -m cyp.backtest.run   --bars 300 --drift 0.001   # 单次回测：收益/回撤/夏普/胜率/盈亏比
python -m cyp.backtest.sweep --bars 300 --top 5         # 批量扫参，按 收益-回撤 排序择优
```
