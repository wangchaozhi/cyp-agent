...
- OrderIntent、提交、成交、保护和关闭事件统一进入 append-only 状态机；文件、内存和 PostgreSQL 适配器均支持先持久化后执行、事件 ID 幂等、冲突拒绝和完整日志重放。
- 服务启动会恢复订单视图；Paper 对进程内不存在的未提交订单执行确定性收尾，OKX Demo 使用原 `clOrdId` 只读查询远端状态，未知结果保持冻结且禁止盲目重下单。
- 人工平仓、自动平仓和反向平仓统一走 reduce-only 持久执行路径；确认空仓并清理保护单后，关闭平仓单以及同币种的原开仓生命周期。
- 文件订单事件按完整终态订单有界压缩，绝不截断未解决订单或留下不可重放的半段历史；PostgreSQL 新增唯一 `event_id` 索引迁移。

### 安全对账与审计

- 新增 `POST /api/reconcile`，只有账户、风险账本、持仓、保护单、保证金率和订单日志全部一致时才解除冻结；并发对账返回冲突，期间出现的新冻结不会被旧结果覆盖。
- 新增 `GET /api/orders` 与 `GET /api/audit/export`，Dashboard 可导出当前安全状态、订单事件和成交账本；对账失败通过既有脱敏告警通道上报。
- Dashboard 安全提示增加“重新对账”操作和明确的进行中/失败反馈；设置页增加订单与成交审计 JSON 导出入口。
- OpenAPI 升级到 1.9.0，并补齐订单事件、对账和审计导出契约；真实 CEX 与非 Demo 执行继续硬禁用。
