...
当前没有可自主行动的“交易员 Agent”。订单意图由 Orchestrator 从已批准提案确定性构造，只能交给进程内 `PaperVenue`、通过模式策略验证的 OKX Demo，或同时通过静态配置、账户就绪、启动对账与 PostgreSQL 单实例租约门禁的 OKX 实盘场所。Agent 永远拿不到交易所写权限，不能解除任何门禁。
...
- 关闭数学自动审批时，Dashboard 审批支持批准、拒绝和修改。
- 开启数学自动审批时，只有 symbol 白名单、风险分、金额、置信度、盈亏比和 Kelly 正期望同时满足才自动通过，否则自动拒绝且不进入 pending。
- 任何修改或硬风控缩量都会在执行前再次运行 `risk.Assess`。
- SafetyState、Kill Switch、mode 和 execution venue 在下单前再次检查。
- Orchestrator 从最终提案构造幂等 `OrderIntent`，交给已通过运行模式策略的执行场所；实盘成交（包括部分成交）必须经持久 journal 和交易所保护单核验。

任何代码都不得把 LLM 输出直接传给 `Venue.Place`。

## Reviewer

`Reviewer.Run` 接收最终提案、执行结果和 run ID，输出 `kind=entry` 的入场执行检查；持仓实际平仓后由 `Reviewer.RunClosed` 输出 `kind=close` 的盈亏复盘：
...
入场检查的 `PNLQuote` 为 0，避免把未实现盈亏误写成结果；平仓复盘写入真实已实现 PnL、追加持久经验并发出 `reviewed` 事件。保护单自动触发的平仓同步仍由运行时监控后续统一接入。
...
- 审批修改后必须再次硬校验。
- 单个密钥或确认变量不能打开实盘；静态门禁、账户就绪、启动对账和账户级单实例租约缺一不可。
