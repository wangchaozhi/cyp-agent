...
cyp-agent 是半自动加密资产交易助手：系统采集市场快照，由多个只读 Agent 并行分析，策略官生成提案，确定性风控和风控官评审后交给人工审批，最终只在 `PaperVenue` 中模拟执行并复盘。
...
5. v0.2.0 编译期硬禁真实下单；非 Paper 场所只读。
