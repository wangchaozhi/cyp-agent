...
cyp-agent 是风控优先的加密资产交易助手：系统采集市场快照，由多个只读 Agent 并行分析，策略官生成提案，经确定性风控、风控官和数学/人工审批后，交给本地 Paper、OKX Demo，或通过生产门禁的 OKX 实盘执行并复盘。
...
5. 默认配置禁止真实下单；OKX 实盘只允许 USDT 线性永续，且静态配置、账户就绪、启动对账和 PostgreSQL 单实例租约缺一不可。
...
| 执行 | Paper 撮合；OKX Demo/实盘 USDT 永续、幂等订单、部分成交收口、原生保护核验、补救与紧急平仓 |
| 服务 | Go REST/SSE、React 静态资源、健康/就绪、指标和优雅停机 |
| 持久化 | memory、原子 JSON file、PostgreSQL |
| 回测 | 合成/历史蜡烛回放、扫参、OOS/PBO/DSR 和统计验证 |

当前真实资金执行仅限通过生产门禁的 OKX USDT 线性永续；不包含提现/转账、托管资金、高频交易和主网链上 swap。链上场所与签名器代码是安全边界骨架，尚未接入应用执行链。
...
- 新实盘场所不得复用 OKX 开关绕过独立安全里程碑；现有生产门禁不得削弱。
